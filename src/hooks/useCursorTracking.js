...
export const useCursorTracking = () => {
  ...
  const currentCanvasRef = useRef({ canvasId: null })
  ...
  const updateCursor = useCallback((position) => {
    ...
    // Wrap updateCursorPosition with canvasId
    throttledCall(position, (pos) => {
      updateCursorPosition(canvasId, pos.x, pos.y)
    })
  }, [throttledCall, canvasId])
  ...
  useEffect(() => {
    ...
    const isCanvasChange = previousCanvasId && previousCanvasId !== canvasId
